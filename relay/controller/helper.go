...
func preConsumeQuota(ctx context.Context, textRequest *relaymodel.GeneralOpenAIRequest, promptTokens int, ratio float64, meta *meta.Meta) (int64, *relaymodel.ErrorWithStatusCode) {
	// If UseRequestAuth is set (e.g. rag/v1 entrypoint with models that shouldn't be billed), skip pre-consume
	if meta != nil && meta.UseRequestAuth {
		logger.Infof(ctx, "skip pre-consume quota (use request auth): user=%d model=%s path=%s", meta.UserId, textRequest.Model, meta.RequestURLPath)
		return 0, nil
	}
...
func postConsumeQuota(ctx context.Context, usage *relaymodel.Usage, meta *meta.Meta, textRequest *relaymodel.GeneralOpenAIRequest, ratio float64, preConsumedQuota int64, modelRatio float64, groupRatio float64, systemPromptReset bool) {
	// if UseRequestAuth is set, skip all post-consume billing/logging
	if meta != nil && meta.UseRequestAuth {
		logger.Infof(ctx, "skip post-consume quota (use request auth): user=%d model=%s path=%s", meta.UserId, textRequest.Model, meta.RequestURLPath)
		return
	}
